...
void drawSignal(const std::vector<int>& signal, const std::string& data, const std::string& label) {
    ...
    const int yMid = windowHeight / 2; // Center of the window
    const int xStart = 50; // Starting x position for signal drawing
    const int bitWidth = 40; // Width per bit
    ...
    while (window.isOpen()) {
        ...
        // Draw encoding type label centered above input data
        sf::Text labelText;
        ...
        // Draw binary data bits below the encoding label
        int x = xStart;
        for (size_t i = 0; i < data.size(); ++i) {
            ...
            bitText.setPosition(x + 10, 70); // Position below encoding label
            window.draw(bitText);
            x += bitWidth;
        }

        // Draw grid lines aligned to bit boundaries
        x = xStart;
        for (size_t i = 0; i <= data.size(); ++i) {
            sf::RectangleShape gridLine(sf::Vector2f(2, windowHeight - 100));
            gridLine.setPosition(x, 50); // Starts from y = 50
...
int main() {
    ...
    std::cout << "Enter choice (1, 2, 3, 4, 5, or 6): ";
    std::cin >> choice;
